...
def test_indexed_hybrid_retrieval_reuses_stored_embeddings(tmp_path):
    ...
    candidate = retriever.select_indexed(
        question="regional revenue",
        workbook_paths={"sales.xlsx": tmp_path / "sales.xlsx"},
        responses=[],
        fit_context=lambda value: value,
        artifacts=[
            {
    ...
    assert candidate is not None
    assert embedding_client.calls == [["regional revenue"]]
    assert candidate.embedding_used is True
    assert candidate.embedding_source == "stored"
    assert candidate.retrieval_audit[0]["embedding_source"] == "stored"
