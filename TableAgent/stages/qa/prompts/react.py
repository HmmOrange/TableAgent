...
- Reach a group through the operators rather than by arithmetic on row numbers. `operators.read_group_data(table_id, group_id)` returns the records a group owns, `operators.find_in_group(table_id, group_id, query)` locates a label inside it, and `operators.intersect_group_with_header(table_id, group_id, header_id)` gives the exact cells where a group crosses a header. Deriving an offset like `group.data_range.start_row - header.data_range.start_row` is a common source of silent off-by-one errors.
- `operators.read_table_as_dataframe(table_id)` adds two metadata columns: `__section__` names the group owning each row, and `__section_label_row__` is True for a group's visible label row. A label row is not a record: exclude it with `df[~df['__section_label_row__']]` before counting rows or aggregating, or pass `drop_group_label_rows=True`. Scope a frame to one section with `operators.group_row_mask(df, table_id, group_id)`, and filter a header inside one section with `operators.filter_in_group(table_id, group_id, header_id, gte=...)`.
- Identifier attributes are `header.id` and `group.id`. `header.header_id` and `group.group_id` do not exist.
