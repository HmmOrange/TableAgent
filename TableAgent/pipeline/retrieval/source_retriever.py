...
class SourceRetriever:
            ...
            try:
                ...
                for index, candidate in enumerate(candidates):
                    ...
                    if stored_compatible and len(stored_vector) == len(query_vector):
                        ...
                        embedding_source = "generated" if candidate_vector is not None else ""
                    if candidate_vector is None:
                        ranked_candidates.append(candidate)
                        continue
                    ranked_candidates.append(replace(
                        candidate,
                        embedding_score=cosine_similarity(query_vector, candidate_vector),
                        embedding_used=True,
    ...
    @staticmethod
    def _artifact_embedding(artifact: dict[str, Any]) -> tuple[tuple[float, ...], str]:
